...
private:
  ...
  double L, Lfw, Lrv, Vcmd, lfw, lrv, steering, u, v,a1,a2;
...
L1Controller::L1Controller() {
  ...
  pn.param("AngleGain", Angle_gain, -8.0);
  pn.param("D_steer", D_steer, -0.0);
  pn.param("GasGain", Gas_gain, 1.0);
  pn.param("baseSpeed", baseSpeed, 2.0);
  ...
  path_sub = n_.subscribe("/move_base/TebLocalPlannerROS/global_plan", 1,
                          &L1Controller::pathCB, this);
  goal_sub =
...
//   L1 = a1 * carVel.linear.x + a2;
  ...
  // {
  //   L1 = L1 = a1 * carVel.linear.x + a2;
  //   }
  // else
  // {
  //   L1= 0.5 * a1 * carVel.linear.x + a2;
  //   }
  // return L1;
  L1 = 0.55;
 }
...
void L1Controller::controlLoopCB(const ros::TimerEvent &) {
  ...
  if (goal_received) 
  {
    ...
    if (foundForwardPt) 
    {
      ...
      // {
      //   cmd_vel.angular.z =
      //     baseAngle + getSteeringAngle(eta) * 1.5 *Angle_gain +
      ...
      // { if(eta>-10 && eta<10)
      //   {
      //      cmd_vel.linear.x = 1.5 * baseSpeed;
      //      }
      //   else
      //   {
...
int main(int argc, char **argv) {
  // Initiate ROS
  ros::init(argc, argv, "L1Controller_v222");
